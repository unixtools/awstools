...
$ENV{AWS_ELB_HOME} = "/local/awstools/ElasticLoadBalancing-1.0.15.1";
$ENV{AWS_CREDENTIAL_FILE} = $ENV{HOME} . "/.awsiam";
$ENV{EC2_PRIVATE_KEY} = $ENV{HOME} . "/.aws-pk.pem";
$ENV{EC2_CERT} = $ENV{HOME} . "/.aws-cert.pem";
$ENV{EC2_HOME} = "/local/awstools/ec2-api-tools-1.5.2.5";
$ENV{AWS_IAM_HOME} = "/local/awstools/IAMCli-1.3.0";
